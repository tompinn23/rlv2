...
static uint8_t* xml_string_clone(struct xml_string* s) {
	...
	uint8_t* clone = mem_zalloc(s->length + 1 * sizeof(uint8_t));
 ...
 * [PRIVATE]
 * 
 * mem_frees the resources allocated by the node
 */
...
static struct xml_string* xml_parse_tag_end(struct xml_parser* parser) {
	 ...
	 */
	struct xml_string* name = mem_alloc(sizeof(struct xml_string));
...
static struct xml_string* xml_parse_content(struct xml_parser* parser) {
	...
	/* Return text
	 */
	struct xml_string* content = mem_alloc(sizeof(struct xml_string));
...
static struct xml_node* xml_parse_node(struct xml_parser* parser) {
	...
	struct xml_node** children = mem_zalloc(1 * sizeof(struct xml_node*));
	children[0] = 0;
	 ...
	 */
		...
		children = mem_realloc(children, (new_elements + 1) * sizeof(struct xml_node*));
...
node_creation:;
	struct xml_node* node = mem_alloc(sizeof(struct xml_node));
	node->name = tag_open;
...
struct xml_document* xml_parse_document(uint8_t* buffer, size_t length) {
	 ...
	 */
	struct xml_document* document = mem_alloc(sizeof(struct xml_document));
...
struct xml_document* xml_open_document(FILE* source) {
	...
	uint8_t* buffer = mem_alloc(buffer_size * sizeof(uint8_t));
	...
	while (!feof(source)) {
		...
		if (buffer_size - document_length < read_chunk) {
			buffer = mem_realloc(buffer, buffer_size + 2 * read_chunk);
...
void xml_document_free(struct xml_document* document, bool free_buffer) {
	...
	if (free_buffer) {
		mem_free(document->buffer.buffer);
	}
	mem_free(document);
}
